...
/*
        ...
        while(i+3 < retreived_key.length)
        {
            ...
            for (var k = 0;k<4;k++)
            {
                wordBlock.push(retreived_key[i+k]);
            }
            var ptext = cipher.decrypt(wordBlock);
            for (var k = 0;k<4;k++)
            {
        ...
        debugger;
        var buff = '';
        for (var i = 0;i<key_map_string.length && key_map_string.charCodeAt(i) != 0;i++)
        {
...
/*
  var key_bits = sjcl.codec.utf8String.toBits(key_str);
    ...
    var lastIndex = key_bits.length - 1;
